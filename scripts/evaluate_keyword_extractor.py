"""
Evaluation script for KeywordBertSumExtractor using ROUGE metrics
"""
...
def evaluate_extractor(extractor, dataset, output_file, max_samples=1000):
    ...
    print("Evaluating KeywordBertSumExtractor...")
    ...
    for i, sample in enumerate(tqdm(dataset[:max_samples])):
        ...
        summary = extractor.generate_summary(content, max_length=3)
    ...
    metrics_file = os.path.join(os.path.dirname(output_file), 'keyword_evaluation_metrics.json')
...
def main():
    ...
    output_file = 'results/evaluation/keyword_evaluation_results.json'
