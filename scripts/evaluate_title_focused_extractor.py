"""
Evaluation script for TitleFocusedExtractor using ROUGE metrics
"""
...
def evaluate_extractor(extractor, dataset, output_file, max_samples=1000):
    """Evaluate extractor on dataset"""
    print("Evaluating TitleFocusedExtractor...")
    ...
    for i, sample in enumerate(tqdm(dataset[:max_samples])):
        ...
        summary = extractor.generate_summary(content, max_length=1)
        ...
        try:
            ...
            summary_sentiment = sentiment_analyzer.analyze(summary)
            
            label_consistency = 1 if content_sentiment['label'] == summary_sentiment['label'] else 0
            
            score_consistency = 1 - abs(content_sentiment['score'] - summary_sentiment['score'])
    ...
    metrics_file = os.path.join(os.path.dirname(output_file), 'title_focused_evaluation_metrics.json')
...
def main():
    ...
    output_file = 'results/evaluation/title_focused_evaluation_results.json'
    evaluate_extractor(extractor, dataset, output_file, max_samples=1000)
